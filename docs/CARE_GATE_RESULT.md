# CARE-Lite method gate and cross-planner evidence

This document records the earlier rule-based deadline gate. Final CARE adds the
exact scenario-separation certificate described in `CARE_ALGORITHM.md`; its
9,600-episode loss/baseline audit is in `CARE_FINAL_RESULTS.md`.
...
- policies: One-shot, Action-triggered, PSR-UT, and deadline-aware CARE-Lite;
...
| A* | CARE-Lite | 0.9375 ± 0.0843 [0.9213, 0.9537] | 68.63 ± 1.27 | 42.14 |
| D* Lite | One-shot | 0.8462 ± 0.1079 [0.8250, 0.8675] | 65.30 ± 1.08 | 142.77 |
| D* Lite | PSR-UT | 0.9313 ± 0.0821 [0.9150, 0.9463] | 75.64 ± 1.40 | 156.07 |
| D* Lite | CARE-Lite | 0.9400 ± 0.0842 [0.9225, 0.9563] | 68.62 ± 1.11 | 156.02 |

Paired CARE-Lite differences at zero delay:
...
or below zero versus PSR-UT, and (c) a positive CSR CI versus One-shot. CARE-Lite
passes all three conditions under both planners.

## Deadline evidence

The mean receiver-local slack is approximately 4.1 steps. At delay 2
(`L_rtt=4`), roughly half of candidate repairs remain feasible; CARE-Lite and PSR-UT
both reach 0.8550 A* CSR, but CARE-Lite saves 11.78 KB. At delay 4 (`L_rtt=8`), no
repair can arrive before the derived deadline. CARE-Lite therefore sends no repair,
...
For CARE-Lite at zero delay, D* Lite minus A* is +0.0025 CSR with 95% CI
...
The Python D* Lite reference is slower on these small maps: CARE-Lite planning takes
156.02 ms per episode versus 42.14 ms with A*. Within planner, CARE-Lite adds 4.36
