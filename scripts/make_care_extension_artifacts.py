...
def make(analysis_directory: Path, table_directory: Path, figure_directory: Path) -> None:
    ...
    for planner in PLANNERS:
        for ablation, label in LABELS.items():
            csr = ablation_lookup[(planner, ablation, "seeker_success_rate")]
            ...
            ablation_rows.append({
                ...
                "delta_seeker_csr": float(csr["mean_difference"]),
                "delta_seeker_csr_ci95_low": float(csr["ci95_low"]),
                "delta_seeker_csr_ci95_high": float(csr["ci95_high"]),
    ...
    for row in ablation_rows:
        lines.append(
            f"| {row['planner']} | {row['ablation']} | {row['delta_seeker_csr']:+.4f} "
            f"[{row['delta_seeker_csr_ci95_low']:+.4f}, {row['delta_seeker_csr_ci95_high']:+.4f}] | "
    ...
    for study in ordered_studies:
        ...
        for planner in planners:
            random_control = study.startswith("random_negative_")
            reliability_metric = (
                "completion_success_rate" if random_control else "seeker_success_rate"
            )
            csr = summary_lookup[(study, planner, "certificate_repair", reliability_metric)]
            ...
            one_shot = paired_lookup[(study, planner, "one_shot_delta", reliability_metric)]
            ...
            extension_rows.append({
                ...
                "reliability_endpoint": (
                    "Overall completion (random control)"
                    if random_control else "Derived seeker CSR"
                ),
                "care_reliability_mean": float(csr["mean"]),
                "care_reliability_std": float(csr["std"]),
                "care_reliability_ci95_low": float(csr["ci95_low"]),
                "care_reliability_ci95_high": float(csr["ci95_high"]),
                ...
                "delta_reliability_vs_one_shot": float(one_shot["mean_difference"]),
                "delta_reliability_ci95_low": float(one_shot["ci95_low"]),
                "delta_reliability_ci95_high": float(one_shot["ci95_high"]),
    ...
    extension_lines = [
        ...
        "Every condition uses 100 matched layouts. Structured studies report exactly derived seeker CSR; random negative controls report overall completion only. Legacy scale rows are tiled-fork stress tests, not natural independent scale layouts.", "",
        "| Study | Planner | Reliability endpoint | CARE reliability mean ± SD [95% CI] | CARE EL mean ± SD [95% CI] | Attempted KB | Δ reliability vs One-shot [95% CI] | ΔEL vs One-shot [95% CI] |",
        "| --- | --- | --- | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in extension_rows:
        extension_lines.append(
            f"| {row['study']} | {row['planner']} | {row['reliability_endpoint']} | "
            f"{row['care_reliability_mean']:.4f} ± {row['care_reliability_std']:.4f} "
            f"[{row['care_reliability_ci95_low']:.4f}, {row['care_reliability_ci95_high']:.4f}] | "
            f"{row['care_el_mean']:.2f} ± {row['care_el_std']:.2f} "
            f"[{row['care_el_ci95_low']:.2f}, {row['care_el_ci95_high']:.2f}] | "
            f"{row['care_attempted_kb_mean']:.2f} | {row['delta_reliability_vs_one_shot']:+.4f} "
            f"[{row['delta_reliability_ci95_low']:+.4f}, {row['delta_reliability_ci95_high']:+.4f}] | "
    ...
    for planner, color in (("astar", "#0072B2"), ("dstar_lite", "#D55E00")):
        points = [summary_lookup[(f"density_{value:02d}", planner, "certificate_repair", "seeker_success_rate")] for value in (10, 20, 30, 40)]
    ...
    axes[1].bar(range(3), [float(summary_lookup[(f"topology_{name}", "astar", "certificate_repair", "seeker_success_rate")]["mean"]) for name in topo], color="#009E73")
    axes[1].set_xticks(range(3), ("T", "Asym.", "Bypass"))
    scale = ((4, 24), (8, 32), (16, 48), (32, 64))
    axes[2].plot([agents for agents, _ in scale], [float(summary_lookup[(f"scale_agents_{agents}_map_{size}", "astar", "certificate_repair", "seeker_success_rate")]["mean"]) for agents, size in scale], marker="o", color="#CC79A7")
    axes[0].set_xlabel("Obstacle density"); axes[1].set_xlabel("Decision topology"); axes[2].set_xlabel("Agents")
    for axis in axes:
        axis.set_ylabel("CARE derived seeker CSR"); axis.grid(alpha=.2)
    axes[0].legend(frameon=False, fontsize=8)
    fig.tight_layout()
    fig.savefig(figure_directory / "care_extensions.png", dpi=220)
    fig.savefig(
        figure_directory / "care_extensions.pdf",
