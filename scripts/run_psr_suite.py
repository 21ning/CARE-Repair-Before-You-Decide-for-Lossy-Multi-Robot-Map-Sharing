...
def run_suite(config: dict, output: Path) -> dict:
    ...
    if layout_seeds is None:
        trial_plan = [(int(seed), int(seed), int(seed)) for seed in config["seeds"]]
    else:
        trial_plan = [
...
def _instance_for_condition(config: dict, *, seed: int, density: float):
    ...
    if family == "random":
        return generate_instance(ExperimentConfig(
            ...
            num_agents=int(config["num_agents"]), observation_radius=int(config["observation_radius"]),
    ...
    if family == "fork_bottleneck_upper_block":
        return generate_fork_bottleneck_instance(
            ...
            observation_radius=int(config["observation_radius"]),
            max_episode_steps=int(config["max_episode_steps"]),
        )
    ...
    if family in {"multifork_t_junction", "multifork_asymmetric_fork", "multifork_narrow_bypass"}:
        return generate_multifork_topology_variant_instance(
            ...
            map_size=int(config["map_size"]), observation_radius=int(config["observation_radius"]),
            max_episode_steps=int(config["max_episode_steps"]),
        )
