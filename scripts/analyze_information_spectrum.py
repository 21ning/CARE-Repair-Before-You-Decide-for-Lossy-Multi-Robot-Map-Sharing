...
def analyze(input_directory: Path, output_directory: Path) -> None:
    ...
    for condition_index, condition in enumerate(sorted(groups)):
        ...
        for metric_index, metric in enumerate(METRICS):
            values = np.asarray([
                metric_value(groups[condition][key], metric) for key in keys
            ])
            ...
            for left_policy, right_policy in comparisons:
                for metric_index, metric in enumerate(METRICS):
                    differences = np.asarray([
                        metric_value(groups[(radius, planner, left_policy)][key], metric)
                        - metric_value(groups[(radius, planner, right_policy)][key], metric)
                        for key in keys
                    ])
                    low, high = bootstrap(
